...
if __name__ == '__main__':
    ...
    configs.dataset_dir = os.path.join('../../', 'dataset', 'new_tracking_dataset')
    # lidar_aug = OneOf([
    ...
    centers = []
    save_folder = '/home/n3xtr/Projects/SFA3D/results/gt_data/'
    ...
    for idx in range(len(dataset)):
        ...
        for box_idx, (cls_id, x, y, z, h, w, l, yaw) in enumerate(labels):
            ...
            if cls_id == 1:
                ...
                cnt_y, cnt_x = tuple(flat)
                data = {
                        'frame': fn_to_be_saved[:-4],
                        'class_id': cls_id,
                        "cnt_x": cnt_x,
                        "cnt_y": cnt_y,
                    }
        ...
        new_column_series = pd.Series(dist, index=df.index[:len(dist)])
        df['distance']= new_column_series
        #df.to_csv(os.path.join(save_folder, fn_to_be_saved), sep=',', index=False)
        
       
        save_path = "/home/n3xtr/result"
        save_file =os.path.join(save_path, img_path[-10:]) 
